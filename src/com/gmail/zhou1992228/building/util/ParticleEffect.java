...
public enum ParticleEffect {
	...
	public static void sendToLocation(ParticleEffect effect, Location location,
		...
		try {
			...
			for (Player player : Bukkit.getOnlinePlayers()) {
				if (location.getWorld().equals(player.getWorld()) &&
					location.distanceSquared(player.getLocation()) < 900) {
					sendPacket(player, packet);
				}
			}
		} catch (Exception e) {
	...
	private static Object createPacket(ParticleEffect effect,
			Location location, float offsetX, float offsetY, float offsetZ,
		...
		Object packet = packetClass.getConstructor(
				new Class[] { String.class, Float.TYPE, Float.TYPE, Float.TYPE,
						Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE,
						Integer.TYPE }).newInstance(
				new Object[] { effect.name,
						Float.valueOf((float) location.getX()),
						Float.valueOf((float) location.getY()),
						Float.valueOf((float) location.getZ()),
